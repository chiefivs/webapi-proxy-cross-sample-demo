        ...
        public static IApplicationBuilder UseWebApiEndpoint<TService>(this IApplicationBuilder appBuilder, string root = null)
        {
            ...
            foreach(var descriptor in descriptors)
            {
                appBuilder.Use(async (context, next) =>
                {
                    ...
                    var acceptType = ContentHelper.GetContentType(context.Request.Headers["Accept"]);
                    if(string.IsNullOrEmpty(acceptType))
                    {
                        context.Response.StatusCode = StatusCodes.Status400BadRequest;
                        var info = new ExceptionInfo(new ArgumentException($"Http request must contain a 'Accept' header '{ContentTypes.Xml}' or '{ContentTypes.Json}'"));
                    ...
                    try
                    {
                        ...
                        if (instanceMtd.ReturnType.Name.StartsWith("Task"))
                        {
                            //CultureInfo.DefaultThreadCurrentCulture = Thread.CurrentThread.CurrentCulture; ;
                            //CultureInfo.DefaultThreadCurrentUICulture = Thread.CurrentThread.CurrentUICulture; ;
                            dynamic task = instanceMtd.Invoke(instance, BindingFlags.Default, null, mtdParams, Thread.CurrentThread.CurrentCulture);
                            task.Wait();
                            result = instanceMtd.ReturnType.GetGenericArguments().Any() ? task.Result : null;
                        }
                        else
                        {
                            result = instanceMtd.Invoke(instance, mtdParams);
                        }
                        context.Response.ContentType = acceptType;

                        context.Response.StatusCode = StatusCodes.Status200OK;

                        if(instanceMtd.ReturnType.Name != "Void" && instanceMtd.ReturnType.Name != "Task")
                        {
                            var text = ContentHelper.SerializeBody(result, acceptType);
