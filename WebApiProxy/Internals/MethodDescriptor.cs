...
namespace WebApiProxy
{
    ...
    internal class MethodDescriptor
    {
        ...
        private MethodDescriptor(string[] interfaceSegments, MethodInfo methodInfo, string root)
        {
            ...
            if (routeAttr != null)
                segments.AddRange(routeAttr.GetSegments());
            else
        ...
        private NameValueCollection GetQueryString(HttpRequest request)
        {
            ...
            #endif
            #if NET46_OR_GREATER
            var queryValues = HttpUtility.ParseQueryString(request.QueryString.ToString());

            var urlSegments = !string.IsNullOrEmpty(request.Path)
                ? request.Path.TrimStart('/').Split('/')
                : new string[] { };
            #endif
    ...
    internal static class RouteAttributeExt
    {
        public static string[] GetSegments(this RouteAttribute route)
        {
            return route.Template.Trim('/').Split('/');
        }
    }
